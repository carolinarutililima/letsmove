...
enum TrailScreenPhase {
    WAITING_FOR_TRIAL_START,
    TRIAL_RUNNING,
    //TRIAL_DONE,
};

class TrialScreen extends Phaser.Scene
{
    ...
    create() {
        ...
        this.announceText = this.make.text( {
            x : 0.5 * vw, 
            y : 0.5 * vh,
            text: `Announcement: Loading ... Move 1m away from the camera 
            `,
            style: {
                color: 'red',
                font: '38px monospace',
        ...
        this.currentPhase = TrailScreenPhase.WAITING_FOR_TRIAL_START;
        this.nextPhase = TrailScreenPhase.WAITING_FOR_TRIAL_START;
    ...
    prevPhase : TrailScreenPhase;
    currentPhase : TrailScreenPhase;
    nextPhase: TrailScreenPhase;

    update( time : number, delta : number ) {
        this.currentPhase = this.nextPhase;

        console.log(`TrialScreen.update ${time} ${delta} ${this.prevPhase} -> ${this.currentPhase} `);

        if ( this.prevPhase !== this.currentPhase ) {
            if ( this.currentPhase === TrailScreenPhase.WAITING_FOR_TRIAL_START ) {
                this.announceText.text = 'Loading Trial ... Move 1m away from the camera ';
                this.announceText.setVisible( true );
                this.announceText.setActive( true ); 


            } else if ( this.currentPhase === TrailScreenPhase.TRIAL_RUNNING ) {
...
//                 setTimeout( () => {
//                     this.nextPhase = TrailScreenPhase.TRIAL_DONE;
//                 }, 30*1000 );
//             } else if ( this.currentPhase === TrailScreenPhase.TRIAL_DONE ) {
...
//                 }, 10*1000 );
        ...
        if ( this.currentPhase == TrailScreenPhase.WAITING_FOR_TRIAL_START  ) {
            if ( ( this.camera != null ) && ( this.jbPoseDetection != null ) && ( this.canvas != null ) ) {
                this.nextPhase = TrailScreenPhase.TRIAL_RUNNING;
            }
        } else if ( this.currentPhase == TrailScreenPhase.TRIAL_RUNNING ) {            
            if ( ( this.camera != null ) && ( this.jbPoseDetection != null ) && ( this.canvas != null ) ) {
 ...
 //       }
    ...
    nextScreen( ) {
        ...
        this.scene.start( 'tomain-screen' );
